...
@app.route("/")
def index():
    return render_template("index.html", min_quality=MIN_QUALITY_SCORE, max_quality=MAX_QUALITY_SCORE)
...
def predict():
    ...
    try:
        features = {name: float(request.form[name]) for name in REQUIRED_FEATURES}
